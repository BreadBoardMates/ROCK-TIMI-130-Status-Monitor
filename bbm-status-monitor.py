...
def getTemp(sensor: str):
	temp = psutil.sensors_temperatures()[sensor][0]
	return round(temp.current)
...
if __name__ == '__main__':
	...
	gtime = up()
	cpu_use = int(psutil.cpu_percent())				#CPU usage
	...
	ram_use = int(psutil.virtual_memory().percent)	#RAM usage
	...
	while True:
		...
		# CPU Temperature
		if cpu_tempG != lastCpuTempG:
			mates.setWidgetValueByIndex(MatesWidget.MATES_MEDIA_GAUGE_B, 0, lastCpuTempG)
		if cpu_tempL != lastCpuTempL:
